 ...
 function fetch_data()
 {
  $.ajax({
   ...
   success:function(data)
   {
    ...
    {
     html +='<tr>';
     html +='<td contenteditable class="column_name" data-column_name="task" data-id="'+data[count].id+'">'+data[count].task+'</td>';
 ...
 $(document).on('click', '#add', function(){
  ...
  if(first_name != '' && last_name != '')
  {
   $.ajax({
    ...
    method:"POST",
    data:{task:task, description:description, _token:_token},
    success:function(data)
    {
     $('#message').html(data);
     fetch_data();
